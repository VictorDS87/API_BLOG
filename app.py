...
@token_obrigatorio
def get_author_from_id(autor, id_autor):
...
def create_new_autor():
    ...
    return jsonify({'mensagem': 'Novo Autor criado!'}, 200)

# Editar o perfil de um autor
@app.route('/autor/<int:id_autor>', methods=['PUT'])
@token_obrigatorio
def change_autor_config(autor, id_autor):
...
@token_obrigatorio
def delete_autor(autor, id_autor):
    autor_existente = Autor.query.filter_by(id_autor=id_autor).first()
    if not autor_existente:
        return jsonify({'mensagem': 'Autor não existe!'})
    db.session.delete(autor_existente)
    db.session.commit()

    return jsonify({'mensagem': 'Autor apagado com sucesso!'})
